...
@app.route('/receive_votes', methods=["GET", "POST"])
def upvote():
    print "request.form", request.form
    upvote_id = request.form['upvote_id']
    post_to_add_point = Post.query.filter_by(id=upvote_id).first()
    post_to_add_point.points += 1    
    return json.dumps({'status':'OK', 'upvote_id': upvote_id, 'title': post_to_add_point.title, 'points':post_to_add_point.points})
